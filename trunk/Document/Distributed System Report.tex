...
\title{Distributed File System Report}
...
\IEEEauthorblockA{ pham0071@ntu.edu.sg}
\and
\IEEEauthorblockN{Jun Zhang}
\IEEEauthorblockA{jzhang027@ntu.edu.sg}
\and
\IEEEauthorblockN{Sibo Wang}
\IEEEauthorblockA{wang0759@ntu.edu.sg}
}
...
We use Java to implement our system design since Java provides RMI model which can facilitate the communication between different nodes.
...
\item Master sending recovering command to slave: Once the master detects dead slaves, the master sends recovering command to alive slaves.  The interface is: recoverBlock(String filename, Hierachical\_codes recoverd,
